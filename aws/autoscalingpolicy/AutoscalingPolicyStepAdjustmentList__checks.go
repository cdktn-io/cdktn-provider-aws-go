...
func (a *jsiiProxy_AutoscalingPolicyStepAdjustmentList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_AutoscalingPolicyStepAdjustmentList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AutoscalingPolicyStepAdjustment:
		val := val.(*[]*AutoscalingPolicyStepAdjustment)
	...
	case []*AutoscalingPolicyStepAdjustment:
		val_ := val.([]*AutoscalingPolicyStepAdjustment)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*AutoscalingPolicyStepAdjustment; received %#v (a %T)", val, val)
		}
	}
