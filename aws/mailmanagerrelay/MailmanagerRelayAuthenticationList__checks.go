...
func (j *jsiiProxy_MailmanagerRelayAuthenticationList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*MailmanagerRelayAuthentication:
		val := val.(*[]*MailmanagerRelayAuthentication)
	...
	case []*MailmanagerRelayAuthentication:
		val_ := val.([]*MailmanagerRelayAuthentication)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*MailmanagerRelayAuthentication; received %#v (a %T)", val, val)
		}
	}
