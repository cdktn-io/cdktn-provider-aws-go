...
func (s *jsiiProxy_S3BucketReplicationConfigurationRulesList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_S3BucketReplicationConfigurationRulesList) validateSetInternalValueParameters(val interface{}) error {
	...
	case []*S3BucketReplicationConfigurationRules:
		val_ := val.([]*S3BucketReplicationConfigurationRules)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
